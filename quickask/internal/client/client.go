// Package client 是 UI 侧的 quickaskd 连接器：
...
// ResolveBackend 按优先级解析 quickaskd.exe 路径：
// explicit 参数 → 环境变量 QUICKASK_BACKEND → 当前 exe 同目录 → cwd → cwd/bin → cwd/../bin。
func ResolveBackend(explicit string) (string, error) {
	...
	add(explicit)
	add(os.Getenv("QUICKASK_BACKEND"))
	if exe, err := os.Executable(); err == nil {
		add(filepath.Join(filepath.Dir(exe), "quickaskd.exe"))
	}
	add("quickaskd.exe")
	add(filepath.Join("bin", "quickaskd.exe"))
	add(filepath.Join("..", "bin", "quickaskd.exe"))
	for _, p := range cands {
	...
	return "", fmt.Errorf("quickaskd.exe not found (tried: %v)", cands)
}
...
func (c *Client) spawn(ctx context.Context) error {
	...
	if err := cmd.Start(); err != nil {
		return fmt.Errorf("start quickaskd (%s): %w", c.path, err)
	}
