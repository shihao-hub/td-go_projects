...
// buildBackendD 构建真实 quickaskd.exe（每个测试进程只构建一次）。
func buildBackendD(t *testing.T) string {
	...
	buildOnce.Do(func() {
		...
		dir, err := os.MkdirTemp("", "quickask-e2e-*")
		if err != nil {
			buildErr = err
			return
		}
		binPath = filepath.Join(dir, "quickaskd.exe")
		cmd := exec.Command("go", "build", "-o", binPath, "./cmd/quickaskd")
		cmd.Dir = root
	...
	if buildErr != nil {
		t.Fatalf("build quickaskd: %v", buildErr)
	}
	return binPath
}
...
func TestE2EHandshakeAndCRUD(t *testing.T) {
	...
	if h := c.Hello(); h.Proto != protocol.Version || h.Name != "quickaskd" {
