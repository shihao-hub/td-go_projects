...
从 `zread-tray`（系统托盘版）复制改造而来，进程管理逻辑（browse 子进程拉起、taskkill 树杀、记住上次工作区）原样继承，托盘/目录选择框替换为子命令与 JSON 输出（模式参考 `docs/projects/go_projects/clictl/Go CLI JSON 输出模式参考.md`）。
...
zread-tray（GUI 版）已归档至父仓 `.archived/projects/go_projects/zread-tray`，由本项目替代；二者不共享运行状态：zreadmanager 有独立的 pidfile 与配置目录。
