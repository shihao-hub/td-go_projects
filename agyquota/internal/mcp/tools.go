...
type quotaGetIn struct {
	Source    string `json:"source,omitempty" jsonschema:"enum=agy,enum=zed,可选：查询数据源：'agy' 为官方 CLI/桌面端（默认），'zed' 为 Zed 编辑器插件账号"`
...
func registerTools(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		...
		src := in.Source
		if src == "" {
			src = service.SourceAgy
		}
		snap, err := mustSvc().GetQuota(ctx, service.Options{
			Source:    src,
