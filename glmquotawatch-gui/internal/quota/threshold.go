...
// 便于表驱动测试。语义与归档版 glmquotawatch 逐条一致（逻辑零改动移植）。
package quota
...
//     （滚动窗口用量自然回落不代表已处理，只有跌破滞回线才重新触发资格）；
//   - prev 的阈值指纹与当前配置不一致时整体清空（配置变更即重新触发）。
func Evaluate(cfg store.Config, prev store.State, limits []api.Limit, now time.Time) (store.State, []Outcome) {
	...
	for _, l := range limits {
		if l.Type != "TOKENS_LIMIT" || l.Percentage == nil {
			continue // 只评估 token 类窗口；percentage 缺失无法评估
		}
		p := *l.Percentage
		key := l.Key()
		out := Outcome{
		...
		case newly > 0:
			...
			// 记账 F 全量（不只 newly），回落时不需要重复触发已越过的档位
