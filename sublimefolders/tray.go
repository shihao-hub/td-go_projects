...
func RunTray(st *Store, interval time.Duration, retainDays int) {
	systray.Run(func() {
		...
		systray.SetTooltip(fmt.Sprintf("sublimefolders · 每 %s 记录一次 Sublime 打开的目录", interval))

		mStatus := systray.AddMenuItem("sublimefolders · 运行中", "定时记录 Sublime Text 打开的目录")
		mStatus.Disable()
		...
		mQuit := systray.AddMenuItem("退出", "退出 sublimefolders")
		...
		go func() {
			for {
				...
				case <-mLatest.ClickedCh:
					...
					if err != nil {
						AlertError("sublimefolders", "查询记录失败: "+err.Error())
						continue
					}
